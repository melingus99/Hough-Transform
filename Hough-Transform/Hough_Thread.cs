...
namespace Hough_Transform
{
    class Hough_Thread
    {
        ...
        public Hough_Thread(Bitmap image)
        {
            this.nrThreads = 3;
            img = image;
        ...
        public Hough_Thread(Bitmap image,int nrThreads)
        {
   ...
   */
        public void initialise(int width, int height)
        {
            ...
            int ratioX = img.Width/ nrThreads;
            int ratioY = img.Height / nrThreads;
            ...
            for(int i = 0; i < nrThreads-1; i++)
            {
                ...
                finishX = (i + 1) * ratioX;
                finishY = (i + 1) * ratioY;
                Thread thread = new Thread(() => this.addPoints(img,startX,startY,finishX,finishY));
                threads.Add(thread);
            }

            startX = nrThreads * ratioX;
            startY = nrThreads * ratioY;
            finishX = img.Width;
            finishY = img.Height;
            Thread threadf = new Thread(() => this.addPoints(img, startX, startY, finishX, finishY));
            threads.Add(threadf);
   ...
   */
        ...
        public void addPoints(Bitmap image,int startX,int startY,int finishX,int finishY)
        {
   ...
   */
        ...
        public void draw()
        {
            ...
            List<HoughLine> lines = this.getLines(9, 30);
